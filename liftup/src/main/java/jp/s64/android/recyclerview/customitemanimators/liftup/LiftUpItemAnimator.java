...
public class LiftUpItemAnimator extends FadeLiftUpItemAnimator {

    public LiftUpItemAnimator(@NonNull RecyclerView target) {
        super(target);
    }

    public LiftUpItemAnimator(@NonNull RecyclerView target, int duration) {
    ...
    protected ViewPropertyAnimator createAddAnimation(
        ...
        if (info.waitAdditionAfterFinishMoves == Boolean.FALSE) {
            ...
            info.holder.itemView.setAlpha(AFTER_ANIMATE_ALPHA);
            ret = animator
                    .translationY(0f);
        } else {
            ret = animator;
